...
def get_leftmost_bbox(annotations,object_name):
    ...
    for annots in annotations:
        print(annots)
        if annots[0] == 0:
            bbox = [annots[2], annots[3], annots[4],annots[5]]
            bboxes.append(bbox)
...
def apply_rotation(pcd, theta_rad):
    ...
    pcd.rotate(R_xy, center=pcd.get_center())
    return pcd, R_xy
...
def auto_voxel_downsample_pair(source_pcd, target_pcd, k=8, scale_factor=2.5):
    """
    Automatically estimates a common voxel size for two point clouds based on 
    their nearest-neighbor distances and applies voxel downsampling.

    Parameters:
        source_pcd (o3d.geometry.PointCloud): Source point cloud.
        target_pcd (o3d.geometry.PointCloud): Target point cloud.
        k (int): Number of neighbors for nearest-neighbor distance estimation (default: 8).
        scale_factor (float): Controls the voxel size; lower values retain more details (default: 2.5).

    Returns:
        source_downsampled (o3d.geometry.PointCloud): Downsampled source point cloud.
        target_downsampled (o3d.geometry.PointCloud): Downsampled target point cloud.
        voxel_size (float): The computed common voxel size.
    """
    
    def compute_voxel_size(pcd, k):
        """Helper function to compute optimal voxel size for a point cloud."""
        points = np.asarray(pcd.points)
        tree = KDTree(points)
        distances, _ = tree.query(points, k=k+1)  # k+1 to exclude self-distance
        nn_distances = distances[:, 1:]  # Remove self-distance
        median_nn = np.median(nn_distances)
        percentile_90 = np.percentile(nn_distances, 90)
        return (median_nn + percentile_90) / scale_factor  # Compute adaptive voxel size
    
    # Compute voxel sizes for both point clouds
    voxel_size_source = compute_voxel_size(source_pcd, k)
    voxel_size_target = compute_voxel_size(target_pcd, k)

    # Use the minimum voxel size to retain more details in both clouds
    common_voxel_size = min(voxel_size_source, voxel_size_target)

    # Apply voxel downsampling with the common voxel size
    source_downsampled = source_pcd.voxel_down_sample(common_voxel_size)
    target_downsampled = target_pcd.voxel_down_sample(common_voxel_size)

    print(f"Common voxel size for registration: {common_voxel_size:.5f}")

    return source_downsampled, target_downsampled, common_voxel_size


def get_line_tip(start_point, end_point):
    """
    Divides a straight line into four equal parts and returns the points 
    with the minimum and second minimum y-values.

    Args:
        start_point: A tuple (x, y) representing the starting point of the line.
        end_point: A tuple (x, y) representing the ending point of the line.

    Returns:
        A tuple of two points (min_y_point, second_min_y_point)
    """
    x1, y1 = start_point
    x2, y2 = end_point
    ...
    points = [(int(x1 + i * dx), int(y1 + i * dy)) for i in range(5)]

    # Sort points by y-value
    sorted_by_y = sorted(points, key=lambda p: p[1])

    # Return the two with lowest y-values
