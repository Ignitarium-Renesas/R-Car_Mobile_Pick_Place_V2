...
class Infer:
    ...
    def infer(self, image, save=False, hide_labels=True):
        ...
        results = self.model.predict(
            source=image, save=save, project=self.output_dir, hide_labels=False, verbose=True, conf=0.3
        )
        prediction_end_time = time.time() - predict_start_time
        print(f"Prediction End Time : {prediction_end_time}")
        post_process = time.time()
        results = self.draw_segmentation(image=image, results=results)
        post_process_time = time.time() - post_process
        print(f"Post processing time: {post_process_time}")
        return results
