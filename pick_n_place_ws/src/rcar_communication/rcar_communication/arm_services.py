...
class Pick_Object(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('server_ip', '192.168.0.145')
        self.declare_parameter('socket_flag', True)
        self.declare_parameter('speed', 60)
        self.declare_parameter('model', 0)
        
        # Declare the home_pose parameter with a default value
        # angle
        self.declare_parameter('home_pose', [-50.00, 106.08, -132.09, 24.78, -10.98, 44.91])
        ...
        if self.mc:
            ...
            tool_offset = [0,0,100,0,0,0]
            self.mc.set_tool_reference(tool_offset)
        ...
        self.get_pick_pose = self.create_service(TransformPose, 'get_pose', self.get_transform_pose_callback,callback_group=self.mutually_callback)
    ...
    def get_transform_pose_callback(self,req, res):
        ...
        res.transformed_pose = [float(pose) for pose in self.transformed_pose]
        return res
    ...
    def set_pick_pose_callback(self,req, res):
        ...
        res.error_code = self.sync_move_to_coords(self.transformed_pose, timeout=1.0)
        if not res.error_code:
            self.get_logger().info(f"Failed to reach Base point. Error code: {res.error_code}")    
            res.error_code = False
            res.is_pose_reachable = False
            return res
        ...
        self.get_logger().info(f"Set pick pose callback, time: {time_diff}")
        return res
    ...
    def set_pose_callback(self,req, res):
        ...
        # Move arm
        res.error_code = self.send_angles(self.transformed_pose)
        if not res.error_code:
        ...
        self.get_logger().info("Base point reached.")
        time_diff = time.time() - start_time
        self.get_logger().info(f"Set pose callback, time: {time_diff}")
        return res
    ...
    def linear_control_callback(self,req,res):
        ...
        transformed_pose = self.transformed_pose[req.axis] if self.transformed_pose != None else self.mc.get_coords()[req.axis]

        move_length = transformed_pose + req.move_length

        res.error_code = self.move_and_check_error(req.axis + 1, move_length)
        if not res.error_code:
        ...
        time_diff = time.time() - start_time
        self.get_logger().info(f"Linear control callback, time: {time_diff}")
        return res
    ...
    def gripper_control_callback(self, req, res):
        ...
        if not gripper_flag:
            error_code = self.mc.set_gripper_state(1, 100, 1)
            res.error_code = True
            if not error_code:
                res.error_code = False
                self.get_logger().error("Failed to close gripper.")
                time_diff = time.time() - start_time
                self.get_logger().info(f"gripper callback, time: {time_diff}")
                return res
            ...
            error_code = self.mc.set_gripper_state(0, 100, 1)
            res.error_code = True
            if not error_code:
                res.error_code = False
    ...
    def sync_move_to_coords(self, coords, timeout=1.0):
        """
    ...
    def send_angles(self, angles):
        ...
        Returns:
            bool: True if the motion succeeds, otherwise False.
        """
        if self.mc.send_angles(angles,self.default_speed):
            if self.mc.get_error_information() == 0:
                return True
        self.get_logger().error("Failed to reach target coordinates.")
        return False
    ...
    def get_pose_about_base(self, pose=MecharmCoords()):
        ...
        if base_pose:
            ...
            if pose.rx != 0.0 or pose.ry != 0.0 or pose.rz != 0.0:
                ...
                # add shift and convert to the roll values that we want.
                roll = -90.0 +  pose.rx
                if roll < -180.0:
                    difference = -180.0 - roll
                    roll = 180 - difference

                elif roll > 180.0:
                    roll = -(roll - 180.0)

                # Thresholding based on angle possible from arm
                if roll < -135:
                    roll = -135

                if pose.rz < 0.0:
                    yaw_difference = -95.0 - yaw
                    yaw = -95.0 + yaw_difference
                ...
                coords_data = [
                    ...
                    (base_pose.position.y + 0.01)*1000,  
                    (base_pose.position.z + 0.005)*1000,
                    -90.0,            # -90 for normal picking....
                    45.0,               #48.0  for notmal picking...
                    -90.0             #-90  for normal picking....
                ]
    ...
    def get_transformed_pose(self, pose, target_frame='joint1', source_frame='camera_link'):
        ...
        try:
            ...
            transform = self.tf_buffer.lookup_transform(
                ...
                timeout=rclpy.duration.Duration(seconds=0.25)  # Timeout duration
            )
    ...
    def validate_coordinates(self, pose, threshold=500):
        """
